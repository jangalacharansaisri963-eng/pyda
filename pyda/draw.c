...
static int parse_color(PyObject* obj, Uint8* r, Uint8* g, Uint8* b, Uint8* a) {
    if (PyTuple_Check(obj) || PyList_Check(obj)) {
        ...
        {
            PyObject *tmp;
            tmp = PySequence_GetItem(obj, 0); *r = (Uint8)PyLong_AsLong(tmp); Py_XDECREF(tmp);
            tmp = PySequence_GetItem(obj, 1); *g = (Uint8)PyLong_AsLong(tmp); Py_XDECREF(tmp);
            tmp = PySequence_GetItem(obj, 2); *b = (Uint8)PyLong_AsLong(tmp); Py_XDECREF(tmp);
            if (size >= 4) {
                tmp = PySequence_GetItem(obj, 3); *a = (Uint8)PyLong_AsLong(tmp); Py_XDECREF(tmp);
            } else {
                *a = 255;
            }
        }
