...
static int parse_color(PyObject* obj, SDL_Color* color) {
    if (PyTuple_Check(obj) || PyList_Check(obj)) {
        ...
        color->r = (Uint8)PyLong_AsLong(PySequence_GetItem(obj, 0));
        color->g = (Uint8)PyLong_AsLong(PySequence_GetItem(obj, 1));
        color->b = (Uint8)PyLong_AsLong(PySequence_GetItem(obj, 2));
        color->a = (size >= 4) ? (Uint8)PyLong_AsLong(PySequence_GetItem(obj, 3)) : 255;
        return 1;
